        ...
        for(int j = 0; j < cli_args.global_column_number; j++)
        {
            ...
            for(int mm = 0; mm < 8; mm++)
            {
                if( ! is_within_grid_lines(i + moore_neighbor_modifiers[mm].lin) || 
                    ...
                    obstacle_grid[i + moore_neighbor_modifiers[mm].lin][j + moore_neighbor_modifiers[mm].col] != EMPTY_CELL)
                    continue;

                aux[i + moore_neighbor_modifiers[mm].lin][j + moore_neighbor_modifiers[mm].col] = FIRE_CELL;
            }
        }
    }
